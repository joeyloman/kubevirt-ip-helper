...
type IPPoolLister interface {
	...
	List(selector labels.Selector) (ret []*kubevirtiphelperk8sbinbashorgv1.IPPool, err error)
	// Get retrieves the IPPool from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kubevirtiphelperk8sbinbashorgv1.IPPool, error)
	IPPoolListerExpansion
}

// iPPoolLister implements the IPPoolLister interface.
type iPPoolLister struct {
	listers.ResourceIndexer[*kubevirtiphelperk8sbinbashorgv1.IPPool]
}

// NewIPPoolLister returns a new IPPoolLister.
func NewIPPoolLister(indexer cache.Indexer) IPPoolLister {
	return &iPPoolLister{listers.New[*kubevirtiphelperk8sbinbashorgv1.IPPool](indexer, kubevirtiphelperk8sbinbashorgv1.Resource("ippool"))}
}
